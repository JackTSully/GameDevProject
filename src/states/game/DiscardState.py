...
class DiscardState(BaseState):
    def __init__(self, state_machine):
        super(DiscardState, self).__init__(state_machine)
        ...
        for event in events:
            ...
            for i, item_card in enumerate(self.player.player_item_deck.cards):
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:  # 1 corresponds to the left mouse button
                    ...
                    if card_rect.collidepoint(self.cursor_position) and event.button == 1 and len(self.player.player_item_deck.cards) > 6:
                        ...
                        if self.item_card_index > 2 and self.item_card_index < 6:
            ...
            if len(self.player.player_item_deck.cards) <= 6:
    ...
    def render(self, screen):
        ...
        over_limit = len(self.player.player_item_deck.cards) - 6
        text = "Item limit reached, please discard "+str(over_limit)+" card(s)."  
        if len(self.player.player_item_deck.cards) == 6:
            text = "Item discarded"     
